...
class TabBasicInfo(wx.Panel):
    ...
    def OnLoadData(self, e):
        ...
        for i in range(0, nProf):
            self.ProfInfoArr.append(ProfInfo(self.ProfInfoPanel, i + 1, constant.XSTART, 50 + i * constant.PROF_DATA_GAP))
    ...
    def OnSetData(self, e):
        ...
        if self.nClassRooms == 0 :
            ...
            for i in range(self.nProfessors) :
                ...
                if self.bClassSet is not True:
                    break

        if self.bClassSet is True:
            print("강의 개설 정보 생성 성공")
